...
class DQNAgent:
    """
    Represents a Deep Q-Networks (DQN) agent that predicts Q(S,A) for given A.
    """
    ...
    def make_model(self):
        ...
        # Layer 1
        layer = Conv2D(filters=6, kernel_size=(3, 3), strides=(1, 1), activation='relu', padding='same', name='conv_1')(input_data)
        layer = MaxPooling2D()(layer)

        # Layer 2
        layer = Conv2D(filters=16, kernel_size=(3, 3), strides=(1, 1), activation='relu', padding='same', name='conv_2')(layer)
        layer = MaxPooling2D()(layer)

        # Layer 3
        layer = Flatten()(layer)

        layer = concatenate([layer, action])

        layer = Dense(16, activation='relu')(layer)   
        layer = Dense(1, activation='linear')(layer)             

        model = Model(inputs=[input_data, action], outputs=layer, name='Minesweeper_Agent')
    ...
    def act(self, state):
        ...
        table = self.adjust_shape(original_table)
        for i in range(0, original_table.shape[0]):
            for j in range(0, original_table.shape[1]):
                if original_table[i, j] == MinesweeperCore.UNKNOWN_CELL:
                    nn_input = [table, np.array([i, j]).reshape(1, 2)]
                    qsa = self.model.predict(nn_input)
                    possible_plays.append((i,j))
                    possible_plays_qsa.append(qsa)
        action = possible_plays[ np.argmax(possible_plays_qsa) ]
    ...
    def replay(self, batch_size):
        ...
        for state, action, reward, next_state, done in minibatch:
            original_table = next_state
            table = self.adjust_shape(next_state)
            # target = self.model.predict(nn_input)
            if not done:
                max_qsa = -inf
                for i in range(0, original_table.shape[0]):
                    for j in range(0, original_table.shape[1]):
                        if original_table[i, j] == MinesweeperCore.UNKNOWN_CELL:
                            ...
                            if qsa > max_qsa:
                                max_qsa = qsa
                target = reward + self.gamma * max_qsa
            else:
                target = reward
